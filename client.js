...
function detectOpeningTheory() {
    ...
    if (typeof MAESTRO_KNOWLEDGE !== 'undefined') {
        ...
        if (MAESTRO_KNOWLEDGE.eco) {
            ...
            if (bestEco) {
                window.lastKnownOpening = bestEco;
                window.lastKnownComments = ["Línea teórica principal."];
                // Si la jugada actual es el final de una línea ECO, es exacto
                if (maxEcoLen === historyLen) {
                    return { name: bestEco, comments: window.lastKnownComments, isExact: true };
                }
            }
        }
    }
    ...
    return { name: "Posición Dinámica", comments: ["Estructura original o medio juego."] };
}
...
function updateCoachDashboard(quality, theory, tactical, bestMove, ev, isMate, moved = false) {
    ...
    // ALWAYS sync to mobile info modal (even if closed)
...
window.toggleBestMovesGlobal = function () {
    ...
    // Force immediate sync of the quick panel
...
window.toggleLogsTicker = function () {
    const isVisible = $('#mobile-logs-ticker-container').is(':visible');
    if (isVisible) {
        $('#mobile-logs-ticker-container').slideUp(200);
        $('#btn-toggle-ticker').removeClass('active');
    } else {
        $('#mobile-logs-ticker-container').slideDown(200);
        $('#btn-toggle-ticker').addClass('active');
    }
};
...
function syncMobileInfo() {
    // 1. EVALUATION
    const evalVal = $('#eval-text-overlay').text() || '0.0';
    $('#mobile-eval-text-modal').text(evalVal);

    const evalWidth = $('#eval-bar-fill').css('width') || '50%';
    $('#mobile-eval-fill').css('width', evalWidth);

    const accuracy = $('#eval-accuracy').text() || '--%';
    $('#mobile-accuracy-modal').text(accuracy);

    const depth = $('#eval-depth').text() || 'Profundidad: --';
    $('#mobile-depth-modal').text(depth);

    // 2. BEST MOVES
    const bestMovesHtml = $('#best-moves-list').html();
    if (bestMovesHtml && bestMovesHtml.trim() !== '') {
        $('#mobile-best-moves-modal').html(bestMovesHtml);
    }

    // 3. MAESTRO INSIGHT
    $('#mobile-maestro-opening-modal').text($('#maestro-opening-name').text());
    $('#mobile-maestro-plan-modal').text($('#maestro-plan').text());

    const trapMsg = $('#maestro-trap').text().trim();
    if (trapMsg !== '' && trapMsg !== '--') {
        $('#mobile-maestro-trap-box-modal').show();
        $('#mobile-maestro-trap-modal').text(trapMsg);
    } else {
        $('#mobile-maestro-trap-box-modal').hide();
    }

    // 4. STATS
    $('#mobile-stat-white-modal').css('width', $('#stat-white').css('width'));
    $('#mobile-stat-draw-modal').css('width', $('#stat-draw').css('width'));
    $('#mobile-stat-black-modal').css('width', $('#stat-black').css('width'));

    $('#mobile-txt-white-modal').text($('#txt-white').text());
    $('#mobile-txt-draw-modal').text($('#txt-draw').text());
    $('#mobile-txt-black-modal').text($('#txt-black').text());

    // 5. COACH / CHAT
    const coachHtml = $('#coach-txt').html();
    if (coachHtml) {
        $('#mobile-coach-logs').html(coachHtml);
        $('#mobile-coach-logs-modal').html(coachHtml);
    }

    // 6. THREATS
    const threatsHtml = $('#threats-list').html();
    if (threatsHtml && threatsHtml.trim() !== '') {
        $('#mobile-threats-list-modal').html(threatsHtml);
    } else {
        $('#mobile-threats-list-modal').html('Ninguna amenaza detectada');
    }
...
function syncQuickBestMoves() {
    // Try to use window.topMoves first (more reliable data)
    if (window.topMoves && window.topMoves.length > 0) {
        const moves = window.topMoves.slice(0, 3).map(m => ({
            san: m.move,
            eval: (m.score > 0 ? '+' : '') + m.score
        }));
        const html = moves.map(m => `<div class="quick-move-pill" onclick="applySuggestedMove('${m.san}')" style="cursor:pointer;">${m.san} <span style="opacity:0.7; font-size:0.6rem;">${m.eval}</span></div>`).join('');
        $('#quick-moves-content').html(html);
        return;
    }

    // Fallback to parsing DOM
    const moves = [];
    $('#best-moves-list .move-item').each(function (i) {
        if (i < 3) {
            const san = $(this).find('.move-san').text();
            const evalText = $(this).find('.move-eval').text();
            if (san) moves.push({ san, eval: evalText });
        }
    });
    ...
    if (moves.length > 0) {
        ...
        $('#quick-moves-content').html('<span style="font-size:0.6rem; opacity:0.5;">Analizando...</span>');
    }
}
...
// NEW: MODERN SEARCH LOGIC FOR OPENINGS
$('#opening-search-input').on('input', function () {
    let query = $(this).val().toLowerCase().trim();
    let $suggestions = $('#opening-suggestions');
    $suggestions.empty();
    ...
    OPENINGS_DATA.forEach((group, gIdx) => {
        let matches = group.items.filter(item =>
            item.name.toLowerCase().includes(query) ||
            group.group.toLowerCase().includes(query)
        );

        if (matches.length > 0) {
            $suggestions.append(`<div class="suggestion-group-label">${group.group}</div>`);
            matches.forEach(item => {
                let iIdx = group.items.indexOf(item);
                let highlightedName = item.name.replace(new RegExp(`(${query})`, 'gi'), '<mark>$1</mark>');
                $suggestions.append(`<div class="suggestion-item" data-val="${gIdx}-${iIdx}">${highlightedName}</div>`);
                foundCount++;
            });
        }
    });
...
$(document).on('click', '.suggestion-item', function () {
    ...
    let text = $(this).text(); // Note: text() will strip <mark>
    $('#opening-search-input').val(text);
    $('#opening-module-sel').val(val).trigger('change');
    $('#opening-suggestions').hide();
});
...
window.showSubMenu = function (id) {
    ...
    if (id === 'home') {
        ...
        // Update Dash Stats
        $('#dash-elo-value').text(localStorage.getItem('chess_user_elo') || "500");
        const puzElo = localStorage.getItem('chess_puz_elo') || "500";
        $('#dash-puz-value').text(puzElo + "🧩");
        loadDashConfig(); // Reload visibility config
    }
